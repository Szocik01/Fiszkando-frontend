...
export default function SingleAnswer(props) {
  ...
  useEffect(()=>{
    if(isModify)
    {
      ...
      {
        console.log(props.answerObjects[id].value, "Ponowne odświezenie danych")
        setWasInputTouched(true);
        SingleAnswerRef.current.children[props.uniqueClass==="question"?0:1].value=props.answerObjects[id].value;
        ...
        {
          console.log(props.answerObjects[id].imageName,`${props.answerObjects[id].url}`)
          setImageUrl(props.answerObjects[id].url);
          setImageName(props.answerObjects[id].imageName);
        }
      }
    }
  },[isModify,id,SingleAnswerRef,hasDataBeenFetched,props.answerObjects,answersAmount]);
  ...
  function photoPreviewHandler(event) {
    ...
    if (event.target.files && event.target.files.length > 0) 
    {
      ...
      {
        const index=props.imagesArray.findIndex((image)=>{ return image.name=== props.answerObjects[id].imageName});
  ...
  function deleteAnswer()
  {
    ...
    console.log(props.answerObjects, "tablica po usunieciu konkretnej odpowiedzi");
  ...
  return (
    <div className={`${style.answerContainer} ${style[props.uniqueClass]}`} ref={SingleAnswerRef}>
      {isModify && !(props.uniqueClass==="question") && <button type="button" onClick={deleteAnswer} className={style.deleteAnswerButton}><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="white"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12 19 6.41z"/></svg></button>}
        ...
        <div className={style.imagePreviewContainer}>
          ...
          <label>
            ...
            {!imageUrl ? (
              ...
              <img className={style.image} src={`${imageUrl}`} alt="Nie można wyświetlić obrazu"/>
            )}
            </div>
          </label>
        </div>
